...
export const Route = createFileRoute("/properties/$propertyId")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) {
      return { meta: [{ title: "Property unavailable | Ocean Atlas" }, { name: "robots", content: "noindex" }] };
    }
          ...
          <div className="absolute inset-x-0 bottom-0">
            <div className="mx-auto max-w-6xl px-4 pb-8 text-primary-foreground">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Badge className="bg-card text-card-foreground hover:bg-card">{property.type}</Badge>
            ...
            <section>
              ...
              <div className="mt-4 space-y-3">
                {property.rooms.map((r) => (
                  <div key={r.id} className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border bg-card p-5">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">
                        Extra guest {money(r.extraGuestRate)} / night · {r.availableUnits} units left
                      </p>
                    </div>
          ...
          <aside id="booking" className="h-fit lg:sticky lg:top-24">
            <div className="rounded-2xl border bg-card p-6 shadow-[var(--shadow-lift)]">
              ...
              <p className="text-display text-3xl font-semibold">
                {money(property.fromPrice)} <span className="text-base font-normal text-muted-foreground">/ night</span>
              </p>
                  ...
                  <li key={s} className="flex flex-1 items-center gap-1">
                    <span
                      className={`flex size-7 shrink-0 items-center justify-center rounded-full text-xs font-semibold ${
                        i <= step ? "gradient-lagoon text-primary-foreground" : "bg-muted text-muted-foreground"
                      }`}
                    >
                      {i + 1}
                    </span>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <Input id="ad" type="number" min={1} max={8} value={adults} onChange={(e) => setAdults(Number(e.target.value))} />
                    </div>
                    <div className="space-y-1.5">
                      <Label htmlFor="ch">Children</Label>
                      <Input id="ch" type="number" min={0} max={6} value={children} onChange={(e) => setChildren(Number(e.target.value))} />
                    </div>
                  </div>
                  <p className="flex items-center gap-2 text-xs text-muted-foreground">
                    <CalendarDays className="size-3.5" />
                    {datesValid ? `${price.nights} nights selected` : "Choose your travel dates to continue"}
                  </p>
              ...
              {step === 1 && (
                <div className="space-y-3">
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            {!fits && <p className="text-xs text-destructive">Too small for {adults + children} guests</p>}
                          </div>
                          <p className="shrink-0 text-sm font-semibold">{money(r.nightlyRate)}</p>
                        </div>
                      </button>
                    );
                  })}
                  <div className="flex gap-2">
                    ...
                    <Button className="flex-1" disabled={!room} onClick={() => setStep(2)}>Continue</Button>
                  </div>
                </div>
              )}
              ...
              {step === 2 && (
                <div className="space-y-3">
                  {property.addons.map((a) => (
                    ...
                    >
                      <Checkbox checked={addonIds.includes(a.id)} onCheckedChange={() => toggleAddon(a.id)} className="mt-0.5" />
                      <span className="flex-1">
                        ...
                        <Badge variant="secondary" className="mt-2">{a.pricing}</Badge>
                      </span>
                    </label>
                  ))}
                  <div className="flex gap-2">
              ...
              {step === 3 && (
                ...
                >
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Textarea id="creq" rows={3} placeholder="Honeymoon, dietary needs, late check-out…" />
                  </div>
                  <div className="flex gap-2">
                  ...
                  <p className="flex items-center gap-1.5 text-xs text-muted-foreground">
                    <ShieldCheck className="size-3.5" /> No payment now — an agent confirms availability first.
                  </p>
                </form>
              )}
              ...
              <div className="space-y-2 text-sm">
                ...
                <Row label={`Accommodation${price.nights ? ` · ${price.nights} nights` : ""}`} value={price.accommodation} />
                {price.extraGuests > 0 && <Row label="Extra guest charges" value={price.extraGuests} />}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-display text-2xl font-semibold">{money(price.total)}</span>
                </div>
                <p className="flex items-center gap-1.5 text-xs text-muted-foreground">
                  <Users className="size-3.5" /> {adults} adults, {children} children · rates in USD, taxes included.
                </p>
              </div>
            </div>
          </aside>
        ...
        <section className="mx-auto max-w-6xl px-4 pb-8">
          ...
          <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {similar.map((p) => (
              ...
              >
                <img src={p.image} alt={p.name} loading="lazy" width={1200} height={800} className="aspect-[16/10] w-full object-cover" />
                <div className="p-4">
                  <p className="font-semibold">{p.name}</p>
                  <p className="text-sm text-muted-foreground">{p.atoll} · from {money(p.fromPrice)}</p>
                </div>
              </Link>
            ))}
          </div>
